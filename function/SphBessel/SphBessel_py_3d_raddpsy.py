...
raddpsi_values = np.zeros_like(kr_values, dtype=complex)
...
for i in range(np.size(kr_values)):
    ...
    raddpsi_values.flat[i] = result['raddpsi'][nmax-1]  # Extract raddpsi

# Reshape the results to match the grid
raddpsi_values = raddpsi_values.reshape(kr_real_mesh.shape)

# Create a 3D plot for raddpsi
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(kr_real_mesh, kr_imag_mesh, np.real(raddpsi_values), cmap='viridis')
ax.set_xlabel('Real part of kr')
ax.set_ylabel('Imaginary part of kr')
ax.set_zlabel('Real part of raddpsi')
ax.set_title('3D Plot of real part of raddpsi for kr = -2-2i to +2+2i (nmax = 2, array = 1, type = ''bessel'')')
plt.xlim(2, -2)
plt.ylim(2, -2)

# Add a colorbar
cbar = plt.colorbar(ax.plot_surface(kr_real_mesh, kr_imag_mesh, np.real(raddpsi_values), cmap='viridis'), ax=ax)
cbar.set_label('Real part of raddpsi')
