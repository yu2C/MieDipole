...
n = 2
rcj_values = np.zeros_like(z_values, dtype=complex)
for i in range(z_values.size):
    _, _, _, drcy = rcbesselc(z_values.flat[i], n)
    rcj_values.flat[i] = drcy[n]  # Use drcy as rcj is not defined in the provided code

# Reshape the results to match the grid
rcj_values = rcj_values.reshape(real_mesh.shape)

# Create a 3D plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(real_mesh, imag_mesh, np.real(rcj_values), cmap='viridis')

# Add a colorbar
fig.colorbar(surf, ax=ax, label='Real part of rcj')

ax.set_xlabel('Real part of z')
ax.set_ylabel('Imaginary part of z')
ax.set_zlabel('Real part of rcj')
ax.set_title('3D Plot of rcj for z = -2-2i to +2+2i (n = 2)')
plt.xlim(-2, +2)
plt.ylim(-2, +2)
#plt.zlim(-1.5, 1.5)
