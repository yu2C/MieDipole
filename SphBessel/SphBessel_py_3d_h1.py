...
h1_values = np.zeros_like(kr_values, dtype=complex)
...
for i in range(np.size(kr_values)):
    ...
    h1_values.flat[i] = result['h1'][nmax-1]  # Extract h1

# Reshape the results to match the grid
h1_values = h1_values.reshape(kr_real_mesh.shape)

# Create a 3D plot for h1
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(kr_real_mesh, kr_imag_mesh, np.real(h1_values), cmap='viridis')
ax.set_xlabel('Real part of kr')
ax.set_ylabel('Imaginary part of kr')
ax.set_zlabel('Real part of h1')
ax.set_title('3D Plot of real part of h1 for kr = -2-2i to +2+2i (nmax = 2, array = 1, type = ''hankel1'')')
plt.xlim(2, -2)
plt.ylim(2, -2)

# Add a colorbar
cbar = plt.colorbar(ax.plot_surface(kr_real_mesh, kr_imag_mesh, np.real(h1_values), cmap='viridis'), ax=ax)
cbar.set_label('Real part of h1')
