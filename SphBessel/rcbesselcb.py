...
# Assuming we've defined rcbesselc and other necessary functions

# Values for the plot
z_values = np.linspace(-2 - 2j, 2 + 2j, 100)  # Modify the range and number of points as needed
rcj_real_values = []
rcj_imag_values = []

# Generate rcj for each z value and collect real and imaginary parts
for z_value in z_values:
    rcj, _, _, _ = rcbesselc(z_value, n=1)
    rcj_real_values.append(np.real(rcj[1]))
    rcj_imag_values.append(np.imag(rcj[1]))

# Plot
plt.figure()
plt.plot(np.real(z_values), rcj_real_values, label='Re(rcj) for n=1')
plt.plot(np.real(z_values), rcj_imag_values, label='Im(rcj) for n=1')
plt.xlabel('Re(z)')
plt.ylabel('Values')
plt.legend()
plt.title('Re(rcj) and Im(rcj) for n=1')
plt.show()
