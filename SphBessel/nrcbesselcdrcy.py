...
z_values = np.linspace(0, 14, 1000)

# Initialize arrays for drcy values for different n values
n_values = [1, 2, 3]
drcy_values = np.zeros((len(n_values), len(z_values)), dtype=complex)
...
for i, n in enumerate(n_values):
    for j, z_real in enumerate(z_values):
        ...
        _, _, _, drcy = rcbesselc(z, n)
        drcy_values[i, j] = drcy[n]  # Python uses 0-based indexing

# Plot drcy for different n values as a function of the real part of z
plt.figure(figsize=(10, 6))
for i, n in enumerate(n_values):
    plt.plot(z_values, drcy_values[i].real, label=f'drcy, n={n}')

plt.xlabel('Real part of z')
plt.ylabel('drcy')
plt.title('Derivative of Riccati-Bessel function (drcy) for z from 0 to 14')
plt.ylim(-2, +2)
plt.xlim(0, 14)
plt.legend()
plt.grid(True)
