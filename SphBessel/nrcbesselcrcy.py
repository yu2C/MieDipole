...
n_values = [1, 2, 3]
rcy_values = np.zeros((len(n_values), len(z_values)), dtype=complex)
...
for i, n in enumerate(n_values):
    for j, z_real in enumerate(z_values):
        ...
        _, rcy, _, _ = rcbesselc(z, n)
        rcy_values[i, j] = rcy[n].real  # Python uses 0-based indexing

# Plot rcy for different n values as a function of the real part of z
plt.figure(figsize=(10, 6))
for i, n in enumerate(n_values):
    plt.plot(z_values, rcy_values[i].real, label=f'rcy, n={n}')

plt.xlabel('Real part of z')
plt.ylabel('rcy')
plt.title('Riccati-Bessel function (rcy) for z from 0 to 14')
plt.xlim(0, 14)
plt.ylim(-2, 2)
plt.legend()
plt.grid(True)
