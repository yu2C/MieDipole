...
psi_real_values = zeros(size(kr_values));
...
for i = 1:numel(kr_values)
    ...
    psi_real_values(i) = real(result.psi(nmax));  % Extract the real part of psi
end

% Reshape the results to match the grid
psi_real_values = reshape(psi_real_values, size(kr_real_mesh));

% Create a 3D plot
figure;
surf(kr_real_mesh, kr_imag_mesh, psi_real_values);
colorbar;
xlabel('Real part of kr');
ylabel('Imaginary part of kr');
zlabel('Real part of psi');
title('3D Plot of real part of psi for kr = -2-2i to +2+2i (nmax = 2, array = 1, type = ''bessel'')');
