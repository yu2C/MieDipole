...
n = 2
csy_values = np.zeros_like(z_values, dtype=complex)
for i in range(z_values.shape[0]):
    for j in range(z_values.shape[1]):
        _, csy = sbesselc(z_values[i, j], n)
        csy_values[i, j] = csy[n]

# Plot the 3D surface with colorbar
fig = plt.figure(figsize=(12, 6))
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(z_imag, z_real, np.abs(csy_values), cmap='viridis', edgecolor='none')
ax.set_xlabel('Imaginary part of z')
ax.set_ylabel('Real part of z')
ax.set_zlabel('|csy(z, n=2)|')
ax.set_title('csy(z, n=2) for z = -2-2i to +2+2i')
ax.view_init(elev=20, azim=-45)  # Adjust view angle for better visualization
ax.set_zlim(0, np.abs(csy_values).max())  # Limit z-axis for better visualization
ax.set_xlim(2, -2)  # Limit z-axis for better visualization
ax.set_ylim(2, -2)  # Limit z-axis for better visualization

ax.invert_xaxis()  # Invert x-axis to match the MATLAB plot

# Add a colorbar
cbar = plt.colorbar(surf, ax=ax, pad=0.1)
cbar.set_label('|csy(z, n=2)|')
