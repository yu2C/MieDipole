...
def SingleGR0(Settings):
    ...
    if 'DRad' not in Settings:
        Settings['DRad'] = SphBessel.SphBessel(rhoD, nmax, 1, 'hankel1')
    
    # Angular Functions
    if 'DNAng' not in Settings:
        Settings['DNAng'] = NormTauPiP.NormTauPiP(nmax, Settings['DPos']['Sph'][1], 'reversed')
    
    if 'DNAngN' not in Settings:
        Settings['DNAngN'] = NormTauPiP.NormTauPiP(nmax, Settings['DPos']['Sph'][1], 'normal')
    
    # Mie Coefficients
    if 'Source' not in Settings:
        Settings['Source'] = SourCoeff.SourCoeff(Settings, "Green's function only")
    
    if 'Layer0' not in Settings:
        if Settings['BC'] == 'sphere':
            Settings['Layer0'] = MieSingle.MieSingle(Settings['nr'], Settings['k0s'], nmax)
    ...
    Temp = {}
    Temp['DVSF'] = VectSphFunc.VectSphFunc(rhoD, nmax, Settings['DRad'], Settings['DNAngN'], emphi)
