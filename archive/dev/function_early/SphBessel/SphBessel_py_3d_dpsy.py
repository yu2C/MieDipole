...
dpsi_values = np.zeros_like(kr_values, dtype=complex)
...
for i in range(np.size(kr_values)):
    ...
    dpsi_values.flat[i] = result['dpsi'][nmax-1]  # Extract the derivative of Z

# Reshape the results to match the grid
dpsi_values = np.reshape(dpsi_values, kr_real_mesh.shape)

# Create a 3D plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(kr_real_mesh, kr_imag_mesh, np.real(dpsi_values), cmap='viridis')
fig.colorbar(surf, ax=ax, shrink=0.5, aspect=10)  # Add colorbar
ax.set_xlabel('Real part of kr')
ax.set_ylabel('Imaginary part of kr')
ax.set_zlabel('Real part of dpsi')
ax.set_title('3D Plot of real part of dpsi for kr = -2-2i to +2+2i (nmax = 2, array = 1, type = ''bessel'')')
plt.xlim(2, -2)
plt.ylim(2, -2)
