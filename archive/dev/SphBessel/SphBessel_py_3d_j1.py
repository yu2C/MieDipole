...
# Rest of your code for MSTA1, MSTA2, envj, sbesselc, rcbesselc, and SphBessel functions
...
j1_real_values = np.zeros_like(kr_values, dtype=float)
...
for i in range(np.size(kr_values)):
    ...
    j1_real_values.flat[i] = np.real(result['j1'][nmax-1])  # Extract the real part of j1

# Reshape the results to match the grid
j1_real_values = np.reshape(j1_real_values, kr_real_mesh.shape)

# Create a 3D plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(kr_real_mesh, kr_imag_mesh, j1_real_values, cmap='viridis')
fig.colorbar(surf, ax=ax, shrink=0.5, aspect=10)  # Add colorbar
ax.set_xlabel('Real part of kr')
ax.set_ylabel('Imaginary part of kr')
ax.set_zlabel('Real part of j1')
ax.set_title('3D Plot of real part of j1 for kr = -2-2i to +2+2i (nmax = 2, array = 1, type = ''bessel'')')
plt.xlim(2, -2)
plt.ylim(2, -2)
# Show the plot
plt.show()
