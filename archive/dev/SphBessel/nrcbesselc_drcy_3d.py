...
real_parts = np.linspace(2, 15, 100)
imaginary_parts = np.linspace(2, 15, 100)
...
drcy_values = np.zeros(z_values.shape, dtype=complex)

# Calculate drcy for z = 2+2i to 15+15i and n = 2
n = 2
for i in range(z_values.shape[0]):
    for j in range(z_values.shape[1]):
        _, _, _, drcy = rcbesselc(z_values[i, j], n)
        drcy_values[i, j] = drcy[n -1]

# Create a 3D plot
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
surf = ax.plot_surface(real_mesh, imag_mesh, drcy_values.real, cmap='viridis')
fig.colorbar(surf, ax=ax, label='Real part of drcy')
ax.set_xlabel('Real part of z')
ax.set_ylabel('Imaginary part of z')
ax.set_zlabel('Real part of drcy')
ax.set_title('3D Plot of drcy for z = 2+2i to 15+15i (n = 2)')
plt.xlim(15, 0)
plt.ylim(15, 0)
