...
real_parts = np.linspace(2, 15, 130)
imaginary_parts = np.linspace(2, 15, 130)
...
for i in range(z_values.shape[0]):
    for j in range(z_values.shape[1]):
        _, rcy, _, _ = rcbesselc(z_values[i, j], n)
        rcy_values[i, j] = rcy[n-1]
...
ax.set_title('3D Plot of rcy for z = +2+2i to +15+15i (n = 2)')
