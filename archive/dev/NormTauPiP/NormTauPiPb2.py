...
def NormTauPiP(nmax, theta, order):
    ...
    # Output a Dictionary
    NAng = {'NPipy': NPi, 'NTaupy': NTau, 'NPpy': NP}
    # wrap the result in .mat
    sio.savemat('./NormTauPiP.mat', mdict=NAng)
    return NAng

theta = np.linspace(1, np.pi, 50)
#theta = np.arange(0, np.pi, 50)

for i in range(0, 50):
    result = NormTauPiP(1, theta[i], 'normal')
    print(result)

    #sio.savemat('./NormTauPiP1.mat', mdict=NAng)
